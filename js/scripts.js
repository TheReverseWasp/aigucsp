$(document).ready(function () {
  ...
  function request_articles() {
    fetch(
      "https://raw.githubusercontent.com/TheReverseWasp/AIG-UCSP/data/data/jsons/articles.json"
    )
      .then((r) => r.text())
  ...
  function request_members() {
    fetch(
      "https://raw.githubusercontent.com/TheReverseWasp/AIG-UCSP/data/data/jsons/members.json"
    )
      .then((r) => r.text())
  ...
  function request_projects() {
    fetch(
      "https://raw.githubusercontent.com/TheReverseWasp/AIG-UCSP/data/data/jsons/projects.json"
    )
      .then((r) => r.text())
  ...
  function fillLastPapers(sorted_articles) {
    ...
    $(".last-articles-item").each(function () {
      ...
      try {
        $(this).append(
          '<h2 class="h5">' + sorted_articles[i].paper_title + "</h2>"
        );
        $(this).append(
          '<p class="mb-0">' +
            sorted_articles[i].conference_journal_title +
            "</p>"
        );
        i++;
      } catch (error) {
  ...
  function fillAllProjects(sorted_projects) {
    ...
    for (i in sorted_projects) {
      projectRow.append(
        '<div class="col-lg-6"><div class="position-relative mb-5" onclick="buildPI(' +
          sorted_projects[i].project_id +
          ')">' +
          '<img class="img-fluid rounded-3 mb-3" src="assets/images/projects/' +
          sorted_projects[i].image_name +
          '" alt="..." />' +
          '<a class="h4 fw-bolder text-decoration-none link-dark stretched-link" href="#!">' +
  ...
  function fillRoleOne(sorted_members) {
    ...
    for (i in sorted_members) {
      if (sorted_members[i].aig_role_id == 1) {
        roleOneDiv.append(
          '<div class="row align-items-center border-3 border-top border-right border-bottom border-left shadow-lg mx-5">' +
            '<div class="col-lg-3"><img class="img-fluid rounded mb-lg-0" src="assets/images/team/' +
            sorted_members[i].image +
            '" alt="..." /></div>' +
            '<div class="col-lg-6">' +
            '<h2 class="fw-bolder ">' +
            ...
            sorted_members[i].aig_role +
            "</br>" +
            sorted_members[i].university_role +
            "</br>" +
            sorted_members[i].education +
            "</br>" +
            "e-mail: " +
            sorted_members[i].e_mail +
            "</br>" +
            "</p>" +
            "</div>" +
            "</div>"
  ...
  function fillRoleThree(sorted_members) {
    ...
    for (i in sorted_members) {
      if (sorted_members[i].aig_role_id == 3) {
        picture = sorted_members[i].image == "" ? "placeholder.jpg" : sorted_members[i].image;
        roleThreeDiv.append(
          '<div class="col-3 my-5 mx-5 py-3 mb-xl-0 border-3 border-top border-right border-bottom border-left shadow-lg">' +
            '<div class="text-center">' +
            '<img class="img-fluid rounded-circle mb-4 px-4" src="assets/images/team/' +
            picture +
            '" alt="..." />' +
            '<h5 class="fw-bolder">' +
            sorted_members[i].name +
      ...
      if (sorted_members[i].aig_role_id == 4) {
        ...
        roleFourDiv.append(
          '<div class="col-2 my-5 mb-xl-0" >' +
            '<div class="text-center">' +
            '<img class="rounded-circle mb-4" src="assets/images/team/' +
            picture +
            '" alt="..." style="height: 120px; width:120px; object-fit: cover;"/>' +
            '<h6 class="fw-bolder">' +
            sorted_members[i].name +
            "</h6>" +
            "</div>" +
    ...
    for (year in article_years) {
      publications.append(
        '<section class="py-0">' +
          ...
          '<div class="row gx-5">' +
          '<div class="col-12 mb-0 text-primary"><h2 class="fw-bolder "> ' +
          article_years[year] +
          "</h2></div>"
      );
      for (i in sorted_articles) {
        if (sorted_articles[i].date == article_years[year]) {
          publications.append(
            "<hr>" +
              ...
              '<h5 class="fw-bolder text-primary mb-2 "> ' +
              sorted_articles[i].paper_title +
              "</h5><br>" +
              '<p class="mb-0"> ' +
              "Conference/Journal: " +
              sorted_articles[i].conference_journal_title +
              "<br>" +
              "Authors: " +
              sorted_articles[i].authors +
              " <br>" +
              "" +
              sorted_articles[i].state +
              " <br>" +
              "" +
      ...
      publications.append("<hr></div>" + "</div>" + "</section>");
    }
  }
